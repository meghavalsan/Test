"""Signature_ami_regr module.

The :mod:`signature` module generates signatures based on anomalies generated
by the :mod:`autogrid.pam.anomaly` module.This special edition is meant to
extract relavant info for fitting a regression model with just LG_PD_10 anomaly.
"""
...
def _parallel_transform(anomalies, tickets, dataset_config, feeder_df, feeders):
    ...
    if not anomalies.empty:
        for feeder in feeders:
            ...
            # create an array of trigger anomalies of this feeder:
            f_rows = f_anoms.loc[f_anoms.Anomaly.isin(triggers)]
            # limit this array to only trigger anomaly, time and signal:
            f_rows = f_rows[['Anomaly','Time','Signal']]
            # loop through the trigger anomaly name and the time:
            for loc_trig_anom, t, signal in f_rows.itertuples(index=False):
                y['TRIGGER_ANOMALY'].append(loc_trig_anom)
                y['PERC_OF_CUS_DOWN'].append(float(signal.split()[6][:-1]))
                y['NUM_OF_METERS'].append(float(signal.split()[-2][1:]))
